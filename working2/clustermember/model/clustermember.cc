...
using namespace std;
	...
	void ClusterMember::StartApplication (void)
	{
		...
		cout<<"Inside clustermember start app"<<Simulator::Now()<<endl;
	...
	void ClusterMember::StopApplication (void)
	{
		...
		cout<<"Inside clustermember stop app"<<Simulator::Now()<<endl;
		if (m_socket != 0)
		{
			m_socket->Close ();
	...
	void ClusterMember::Setup (Ipv4Address address, uint16_t port, DataRate dr, bool toSend, bool broadcastaddr, uint32_t nodeid, string slaveString,NodeContainer nodesC, Ipv4InterfaceContainer interf,Ptr<ClusterManager> clusMgr,int pkt_type,int topic, int round_start, int round_end, int round, int numRounds, int round_duration)
	{
		peerAddress = address;
		clusterMgr = clusMgr;
		nodeID = nodeid;
	...
	{		
		...
		{
			cout<<"Inside clustermember handle read "<<endl;
			...
			if(pkt_Type == 1)
			{
				...
				if(clusterMgr->isMaster(GetNode()))
				{
					...
					if(clusterMgr->getNumberOfClusters() == 1)
					{
						...
						masterApp->Setup (Ipv4Address::GetBroadcast (), 10, DataRate ("1Mbps"), true, true,nodeID,slaveStr, nodes,interfaces,clusterMgr,packet_type, currentTopic, roundStart, roundEnd , rnd, numRnds, rnd_dur);
						...
						for(std::vector<int>::size_type i = 0; i != slaves.size(); i++) {
							...
							int packet_type = 2;
							peers[i]->Setup (Ipv4Address::GetBroadcast (), peerPort, DataRate ("1Mbps"), false, false, slaveID, slaveStr, nodes,interfaces,clusterMgr,packet_type,currentTopic, roundStart, roundEnd, rnd, numRnds, rnd_dur);
					...
					else
					{
						...
						for(std::vector<int>::size_type i = 0; i != slaves.size(); i++) {
							...
							masterApp[i]->Setup (interfaces.GetAddress (slaveID), peerPort, DataRate ("1Mbps"), true, false,nodeID,slaveStr, nodes,interfaces,clusterMgr,packet_type,currentTopic,roundStart, roundEnd , rnd, numRnds, rnd_dur);
							nodes.Get(nodeID)->AddApplication (masterApp[i]);
						
							masterApp[i]->SetStartTime (Simulator::Now());
							masterApp[i]->SetStopTime (Seconds (roundEnd));
							
							peers[i] = CreateObject<ClusterMember> ();
							int packet_type = 2;
							peers[i]->Setup (interfaces.GetAddress (nodeID), peerPort, DataRate ("1Mbps"), false, false, slaveID, slaveStr, nodes,interfaces,clusterMgr,packet_type,currentTopic, roundStart, roundEnd, rnd, numRnds, rnd_dur);
							nodes.Get(slaveID)->AddApplication (peers[i]);
							peers[i]->SetStartTime (Simulator::Now());
							peers[i]->SetStopTime (Seconds (roundEnd));
					...
					if(clusterMgr->numofMastersrecv == clusterMgr->getNumberOfClusters())
					{
						...
						//start the next round in vanet
						
						Vanet::masterIDs.erase (Vanet::masterIDs.begin(),Vanet::masterIDs.end());
						Vanet::slaveListStrings.erase (Vanet::slaveListStrings.begin(),Vanet::slaveListStrings.end());	
						clusterMgr->numofMastersrecv = 0;
						std::cout<<"*******"<<rnd_dur<<" ** "<<rnd<<" ** "<< roundStart<<" ** "<<roundEnd<<std::endl;
						if(rnd <= numRnds && roundEnd < Vanet::duration)
							Simulator::Schedule(Seconds(rnd_dur),&Vanet::performRound, rnd+1, rnd_dur, roundStart + rnd_dur + 2.0, roundEnd + rnd_dur);
				
						//Simulator::ScheduleNow(&performRound, 1, round_start, round_end, round_duration);
					}	
...
cout<<"Inside clustermember send "<<endl;
